...
#### Codec Implementations (Internal)
...
```
Domain Object  ←→  DataSourceNode  ←→  IDataSourceIoGateway  ←→  File
     ↑ registry.Write/Read ↑
```

`SndWorld` exposes `ConverterRegistry` and `DataSourceIo`. Framework code should read/write files through `IDataSourceIoGateway` (for example in `PersistentBlackboard`, template loading, and auto-initialization).
...
| `SndWorld` | property | Strategy pool, type mapping, DataSourceIo, ConverterRegistry |
...
1. **Copies shared references** from the foreground level — `SndWorld` (strategy pool, mappings), `ProgressBlackboard`, `IFileSystem`, `SaveRootPath`
...
| SessionBlackboard isolation | Each session gets a new `IBlackboard` instance; data restored from `LevelPayload.SessionNode` |
