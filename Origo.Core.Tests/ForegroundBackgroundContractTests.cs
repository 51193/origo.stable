...
public class ForegroundBackgroundContractTests
{
    ...
    [Fact]
    public void CreateBackgroundSessionFromPayload_ReturnsISessionRun_NotConcreteType()
    {
        var (ctx, _) = CreateContext();
        SetupForegroundSession(ctx);
        ...
        using var bg = ctx.SessionManager.CreateBackgroundSessionFromPayload("bg", "bg", payload);
        Assert.IsAssignableFrom<ISessionRun>(bg);
    }
    ...
    public void RoundTrip_SerializeAndLoad_IdenticalBetweenForegroundAndBackground()
    {
        ...
        // 另一个后台反序列化
        using var bg2 = ctx.SessionManager.CreateBackgroundSessionFromPayload("bg2", "level_a", payload);
