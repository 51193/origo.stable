...
public class SavePathPolicyContractTests
{
    ...
    [Fact]
    public void RunDependencies_DefaultStorage_Uses_Injected_SavePathPolicy()
    {
        // Arrange: inject path policy to RunFactory without explicit storage service.
        var logger = new TestLogger();
        var host = new TestSndSceneHost();
        var runtime = TestFactory.CreateRuntime(logger, host);
        var fs = new TestFileSystem();
        ...
        // Act: write through the factory's storage service
        var payload = new LevelPayload
        {
            LevelId = "faclvl",
            SndSceneJson = "[]",
        ...
        var currentDir = customPolicy.GetCurrentDirectory();
        factory.StorageService.WriteLevelPayloadOnly(currentDir, payload);

        // Assert
        var expectedDir = customPolicy.GetLevelDirectory(currentDir, "faclvl");
        var expectedFile = $"root/{customPolicy.GetLevelSndSceneFile(expectedDir)}";
        Assert.True(fs.Exists(expectedFile),
            $"Expected RunFactory-created storage to use custom policy; file at '{expectedFile}'.");
    }
    ...
    public void DefaultSaveStorageService_WriteSavePayloadToCurrentThenSnapshot_Uses_PathPolicy()
    {
        ...
        storage.WriteSavePayloadToCurrentThenSnapshot(payload, "000", "001", logger);
    ...
    public void DefaultSaveStorageService_SnapshotCurrentToSave_Uses_PathPolicy()
    {
        ...
        storage.SnapshotCurrentToSave("000", "snap1");
    ...
    private static void SetupForegroundSession(SndContext ctx)
    {
        ...
        var progressRun = ctx.RunFactory.CreateProgressRun("001", progressBb);
