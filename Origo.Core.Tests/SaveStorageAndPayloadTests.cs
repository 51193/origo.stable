...
public class SaveStorageAndPayloadTests
{
    ...
    [Fact]
    public void SaveStorageFacade_SnapshotCurrentToSave_WhitespaceBaseSaveId_Throws()
    {
        var fs = new TestFileSystem();
        Assert.Throws<ArgumentException>(() => SaveStorageFacade.SnapshotCurrentToSave(fs, "root", "  ", "b"));
    }
    ...
    {
        var fs = new TestFileSystem();
        Assert.Throws<ArgumentException>(() => SaveStorageFacade.SnapshotCurrentToSave(fs, "  ", "a", "b"));
    }
    ...
    {
        var fs = new TestFileSystem();
        Assert.Throws<ArgumentException>(() => SaveStorageFacade.SnapshotCurrentToSave(fs, "root", "a", "  "));
    }
    ...
    public void WriteSavePayloadToCurrentThenSnapshot_NullLogger_Throws()
    {
        ...
        Assert.Throws<ArgumentNullException>(() =>
            SaveStorageFacade.WriteSavePayloadToCurrentThenSnapshot(fs, "root", payload, "0", "1", null!));
    }
    ...
    public void WriteSavePayloadToCurrentThenSnapshot_WhenSnapshotFails_LogsError_LeavesMarkerAndUpdatedCurrent()
    {
        ...
        Assert.Throws<InvalidOperationException>(() =>
            SaveStorageFacade.WriteSavePayloadToCurrentThenSnapshot(fs, "root", payload, "000", "new", logger));
