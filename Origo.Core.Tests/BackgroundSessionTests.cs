...
public class BackgroundSessionTests
{
    ...
    public void DisposedSession_ThrowsOnAllPublicMethods()
    {
        ...
        Assert.Throws<ObjectDisposedException>(() => bg.SceneHost);
        Assert.Throws<ObjectDisposedException>(() => ((SessionRun)bg).PersistLevelState());
    }
    ...
    public void SerializeToPayload_ReturnsLevelPayload_WithCorrectLevelIdAndData()
    {
        ...
        var payload = ((SessionRun)bg).SerializeToPayload();
    ...
    public void LoadFromPayload_RestoresSessionState()
    {
        ...
        using (var source = ctx.SessionManager.CreateBackgroundSession("src_level", "src_level"))
        {
            ...
            payload = ((SessionRun)source).SerializeToPayload();
        }
        ...
        ((SessionRun)target).LoadFromPayload(payload);
    ...
    public void SerializeToPayload_ThenLoadFromPayload_RoundTrips()
    {
        ...
        ((SessionRun)restored).LoadFromPayload(payload);
    ...
    public void LoadFromPayload_Throws_WhenDisposed()
    {
        ...
        Assert.Throws<ObjectDisposedException>(() => ((SessionRun)bg).LoadFromPayload(new LevelPayload()));
    }
    ...
    public void SerializeToPayload_Throws_WhenDisposed()
    {
        ...
        Assert.Throws<ObjectDisposedException>(() => ((SessionRun)bg).SerializeToPayload());
    }

    // ── CreateBackgroundSessionFromPayload ─────────────────────────────

    [Fact]
    public void CreateBackgroundSessionFromPayload_RestoresFromPayload()
    {
        var events = new List<string>();
        var (ctx, _) = CreateForegroundContext(world =>
        {
        ...
        // Create a background session directly from payload.
        events.Clear();
        using var restored = ctx.SessionManager.CreateBackgroundSessionFromPayload("restored_level", "restored_level", payload);
    ...
    [Fact]
    public void CreateBackgroundSessionFromPayload_Throws_WhenPayloadNull()
    {
        var (ctx, _) = CreateForegroundContext();

        Assert.Throws<ArgumentNullException>(() => ctx.SessionManager.CreateBackgroundSessionFromPayload("bg", "bg", null!));
    }
    ...
    public void BuildSavePayload_IncludesBackgroundSessionsInPayload()
    {
        ...
        // BackgroundLevelIds should be persisted in progress blackboard.
        var (found, bgIds) = ctx.ProgressBlackboard!.TryGet<string>(WellKnownKeys.BackgroundLevelIds);
        Assert.True(found);
    ...
    public void SaveAndLoad_RoundTrips_BackgroundSessions()
    {
        ...
        ctx.RunFactory.StorageService.WriteSavePayloadToCurrentThenSnapshot(
            payload, "save_bg_test", "save_bg_test", ctx.Runtime.Logger);
        ...
        var newProgressRun = ctx.RunFactory.CreateProgressRun("save_bg_test", new Blackboard.Blackboard());
    ...
    public void BuildSavePayload_WithNoBackgroundSessions_ClearsBackgroundLevelIds()
    {
        ...
        // Set a stale value.
        ctx.ProgressBlackboard!.Set(WellKnownKeys.BackgroundLevelIds, "old=data");

        var progressRun = ctx.EnsureProgressRun();
        var payload = progressRun.BuildSavePayload("save_empty");

        // BackgroundLevelIds should be cleared (empty string).
        var (found, bgIds) = ctx.ProgressBlackboard!.TryGet<string>(WellKnownKeys.BackgroundLevelIds);
        Assert.True(found);
        Assert.Equal(string.Empty, bgIds);
    ...
    public void SaveAndLoad_RoundTrips_SyncProcessFlag()
    {
        ...
        var newProgressRun = ctx.RunFactory.CreateProgressRun("save_sync_rt", new Blackboard.Blackboard());
        ctx.SetProgressRun(newProgressRun);
        newProgressRun.LoadFromPayload(payload);
        ...
        var (found2, bgIds2) = newProgressRun.ProgressBlackboard.TryGet<string>(WellKnownKeys.BackgroundLevelIds);
        Assert.True(found2);
    ...
    public void SaveAndLoad_FromDisk_RestoresBackgroundSessions()
    {
        ...
        ctx.RunFactory.StorageService.WriteSavePayloadToCurrentThenSnapshot(
            payload, "save_disk_test", "save_disk_test", ctx.Runtime.Logger);
        ...
        var readPayload = ctx.RunFactory.StorageService.ReadSavePayloadFromSnapshot(
        ...
        var newProgressRun = ctx.RunFactory.CreateProgressRun("save_disk_test", new Blackboard.Blackboard());
        ctx.SetProgressRun(newProgressRun);
    ...
    public void ReadFromCurrent_IncludesAllLevelDirectories()
    {
        ...
        ctx.RunFactory.StorageService.WriteSavePayloadToCurrent(payload);

        // Read back from current/ — should include both levels.
        var readPayload = ctx.RunFactory.StorageService.ReadSavePayloadFromCurrent(
        ...
        Action<SndWorld>? configureWorld = null)
    {
        ...
        var progressBb = new Blackboard.Blackboard();
        var progressRun = ctx.RunFactory.CreateProgressRun("test_save", progressBb);
