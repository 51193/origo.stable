...
flowchart TB
    ...
    subgraph core ["Origo.Core"]
        ...
        DS["DataSource<br/>(IDataSourceIoGateway / ConverterRegistry)"]
    end
...
OrigoDefaultEntry（Godot 入口节点）
  └─ OrigoAutoHost（创建 Runtime + 注入适配器）
       └─ OrigoRuntime
            ├─ SndWorld（StrategyPool / TypeMapping / Mappings / DataSourceIo / ConverterRegistry）
...
`ProgressRun` 不再在 `ProgressBlackboard` 中单独存储 active level 键。
前台关卡由 `WellKnownKeys.SessionTopology`（`__foreground__=levelId=syncProcess`）解析得到。
...
```
领域对象  ←→  DataSourceNode  ←→  IDataSourceIoGateway  ←→  文件
     ↑ registry.Write/Read ↑
```

`SndWorld` 暴露 `ConverterRegistry` 与 `DataSourceIo`；框架代码涉及文件读写时应通过 `IDataSourceIoGateway`（例如 `PersistentBlackboard`、模板加载、自动初始化路径）。
...
| `SndWorld` | 属性 | 策略池、类型映射、DataSourceIo、ConverterRegistry |
...
1. **拷贝共享引用** —— 从前台关卡获取 `SndWorld`（策略池、映射）、`ProgressBlackboard`、`IFileSystem`、`SaveRootPath`
...
| SessionBlackboard 隔离 | 每个会话获得新的 `IBlackboard` 实例；数据从 `LevelPayload.SessionNode` 恢复 |

`syncProcess` 只控制会话是否参与 `SessionManager.ProcessAllSessions(...)` 这条批量帧更新通道。
前台条目会刻意序列化为 `__foreground__=levelId=false`；这表示“前台不通过后台批处理通道更新”，而不是“前台不更新”。前台更新由运行时/引擎前台场景主循环驱动。
...
| `ctx.SessionManager.ProcessAllSessions(delta, includeForeground?)` | `ISessionManager` 方法 | 处理 `syncProcess=true` 的会话一帧（主要用于后台会话） |
...
```csharp
using var node = dataSourceIo.ReadTree(filePath);
...
dataSourceIo.WriteTree(filePath, node);
// 节点在此处被释放
```
...
public sealed class ConfigCache : IDisposable
{
    ...
    public ConfigCache(IDataSourceIoGateway dataSourceIo)
    {
    ...
    public void Load(string filePath)
    {
        ...
        _root = _dataSourceIo.ReadTree(filePath);
    }
